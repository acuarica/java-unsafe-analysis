...
#d = rbind(load.csv('csv/unsafe-maven-jdk8.csv'), load.csv('csv/unsafe-maven.csv') );
#d = dcast(d, name+desc~'cs', value.var='name', fun.aggregate=length);
#d = dcast(d, name~'ov', value.var='cs', fun.aggregate=length);
#d = d[d$ov > 1,];
#d = merge(d, df.methods, by.x='name', by.y='method', all.x=TRUE, all.y=TRUE);

csv = load.csv('build/cs-jdk8.csv');
csv = merge(csv, df.methods, by.x='name', by.y='method', all.x=TRUE, all.y=TRUE);

plotoverview = function(df, outfile) {
  df = dcast(df, name+group+package~'cs', value.var='cs', fun.aggregate=sum);
  #df = merge(df, df.methods, by.x='name', by.y='method', all.x=TRUE, all.y=TRUE);
  df[is.na(df$cs),]$cs = 0;

  p = ggplot(df, aes(x=name, y=cs, fill=package))+
...
df[!is.na(df$package) & df$package=='java/util/concurrent/atomic',]$package = 'java/util/concurrent';
df[!is.na(df$package) & df$package=='java/util/concurrent/locks',]$package = 'java/util/concurrent';
